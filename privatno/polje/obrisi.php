...
if(isset($_GET["sifra"])){
	...
	$izraz=$veza->prepare("select count(a.kultura) from kulturaugodini a 
					   	   inner join polje b on a.polje=b.sifra
						   where a.polje=:sifra");
	$izraz->execute($_GET);
...
if(isset($_POST["obrisi"])){
		...
		$izraz=$veza->prepare("delete from polje
		where sifra=:sifra");
		$izraz->execute(array(
		"sifra" => $_POST["sifra"]
		));
	...
	<body>
		...
		<div class="row">
			...
			<div class="large-6 columns">
				...
				<?php else:?>
					<div class="brisanjep">Polje se ne može obrisati jer u sebi sadrži kulturu!</div>
				<?php endif;?>
			</div>
		</div>
